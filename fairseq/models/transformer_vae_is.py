...
def load_mrf_features(single_energy_file, pair_energy_file):
    ...
    single_energy = torch.load(single_energy_file).to(device)  # [length * 20, 1]
    pair_energy = torch.load(pair_energy_file).to(device)   #  [length * length * 400, 1]
    ...
    pair_features = torch.FloatTensor(length, 20, (length - 1) * 20)
    ...
    m = nn.Embedding(num_features, feature_length)
    m.weight = features
...
class TransformerVAEDecoder(TransformerDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, output_projection=None):
        ...
        # self.mapping_layer = nn.Linear(self.args.decoder_embed_dim + self.args.latent_dimension, self.args.decoder_embed_dim)
        ...
        self.mrf_map_layer = nn.Linear(self.feature_length, self.args.decoder_embed_dim)
    ...
    def forward(
        ...
        mrf_ids = torch.tensor([range(slen-2) * 20]) + (prev_output_tokens[:, 1: -1] - 4)  # [batch, len - 2]
        ...
        x[:, 1: -1] = x[:, 1: -1] + trans_mrf_features
...
class TransformerVAEModel(TransformerModel):
    ...
    def __init__(self, args, encoder, decoder):
        ...
        # self.mapping_layer = nn.Linear(self.latent_dim, self.args.max_length * self.args.memory_dimension)
