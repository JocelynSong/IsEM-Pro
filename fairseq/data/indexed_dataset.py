...
def make_dataset(path, impl, fix_lua_indexing=False, dictionary=None, source=True):
    ...
    elif impl == "binarize" and IndexedBinarizedDataset.exists(path):
        return IndexedBinarizedDataset(path, dictionary)
    elif impl == "unsupervised_binarize" and UnsupervisedIndexedBinarizedDataset.exists(path):
        return UnsupervisedIndexedBinarizedDataset(path, dictionary)
    return None
...
class IndexedBinarizedDataset(FairseqDataset):
    """
    Takes a binarized file as input.
    Original lines are also kept in memory
    """
    ...
    def __init__(self, path, dictionary, append_eos=True, reverse_order=False):
        ...
        # self.mask_index = dictionary.index("<mask>")
        # self.max_len = 860
        self.reverse_order = reverse_order
        self.read_data(path)
        self.size = len(self.tokens_list)
    ...
    def read_data(self, path):
        # append eos token when loading data
        with tf.io.gfile.GFile(path, 'rb') as reader:
            data = torch.load(io.BytesIO(reader.read()))

        # we remove sentences which contains 0 (<bos>)
        # this is due to the constraint of fairseq.data.denoising_dataset.py
        # line 205: assert (source[1:-1] >= 1).all()
        zero_num = 0
        sentences = data["sequences"]
        this_sentences = sentences

        for sentence in this_sentences:
            if 0 in sentence:
                zero_num += 1
                continue
            # fit to finetune from pretrained model
            # if self.dictionary[sentence[1]] == "1":
            #     fitness = 1
            # else:
            #     fitness = 0
            sentence = np.insert(sentence[3: ], 0, sentence[1])

            if self.append_eos:
                sentence = np.append(sentence, self.eos_index)
            # if (len(sentence)-2) < self.max_len:
            #     length = self.max_len+2-len(sentence)
            #     for _ in range(length):
            #         sentence = np.insert(sentence, -1, self.mask_index)
        ...
        self.sizes = np.array(self.sizes)
        if zero_num != 0:
            print(f"{path} contains {zero_num} zeros, the item will be ignored.")
...
class UnsupervisedIndexedBinarizedDataset(FairseqDataset):
    """
    Takes a binarized file as input.
    Original lines are also kept in memory
    """
